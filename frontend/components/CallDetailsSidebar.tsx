    ...
    <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50">
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-6">
          <h4 className="font-medium text-gray-800 mb-2">Live Transcript</h4>
          <div className="space-y-1 text-sm text-gray-700 max-h-64 overflow-y-auto border rounded p-2 bg-gray-50">
            {call.transcriptChunks?.length > 0 ? (
              call.transcriptChunks.map((chunk, index) => (
                <div
                  key={index}
                ...
                >
                  <strong className="capitalize">{chunk.role}:</strong>{' '}
                  {chunk.message}
                </div>
              ))
            ) : (
              <p className="text-gray-400 italic">No live transcript yet.</p>
            )}
          </div>
          ...
          {call.transcript && (
            <>
              ...
              <div className="whitespace-pre-wrap text-sm text-gray-600 font-mono border rounded p-2 bg-white">
