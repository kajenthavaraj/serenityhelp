...
class WebSocketService {
  ...
  connect(url: string = "http://localhost:5001") {
    ...
    try {
      ...
      // ✅ NEW: live transcript streaming from backend
      this.socket.on("live_transcript_update", (data: any) => {
        console.log("🟢 Received live transcript update:", data);
        this.emit("liveTranscriptUpdate", data); // PascalCase to keep frontend clean
      });
    } catch (error) {
